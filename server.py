...
def DogeChat(client, username, groupname):
	while True:
...
def handshake(client):
	...
	if groupname in groups:
		if username in groups[groupname].allMembers:
			...
			groups[groupname].sendMessage(username+" has requested to join the group.","DogeChat")
			client.send(b"/wait")
			print("Join Request:",username,"| Group:",groupname)
		threading.Thread(target=DogeChat, args=(client, username, groupname,)).start()
	else:
